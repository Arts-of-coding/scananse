...
# )
# 
# pdf(paste(resultsdir, "88.clustering_resolution.pdf", sep = '/'), width = 75, height = 30)
# klus
# dev.off()
...
seur_obj$costum_clustering <- plyr::mapvalues(x = seur_obj$RNA_snn_res.0.2, from = current.cluster.ids, to = new.cluster.ids)
...
#pdf(paste(resultsdir, "8b.clustering_costum.pdf", sep = '/'), width = 10, height = 8)
#p1 <- DimPlot(seur_obj, label=FALSE, group.by = 'orig.ident')
#p1 <- DimPlot(seur_obj, label=TRUE) 
#p2 <- DimPlot(seur_obj, label=TRUE, group.by = 'costum_clustering') + ggtitle("Louvain Clustering") 
#p3 <- ggplot(seur_obj@meta.data, aes(costum_clustering))+geom_bar(stat="count")+ theme(axis.text.x = element_text(angle = 90))
#print(p1)
#print(p2)
#print(p3)
...
saveRDS(seur_obj, file = "/ceph/rimlsfnwi/data/moldevbio/zhou/jarts/R/scRNA-seq/lakoRNAunannotated2.rds") # 30 PC
#seur_obj <- readRDS("/ceph/rimlsfnwi/data/moldevbio/zhou/jarts/R/scRNA-seq/lakoRNAunannotated2.rds") # 10 PC
seur_obj <- readRDS("/ceph/rimlsfnwi/data/moldevbio/zhou/jarts/R/scRNA-seq/lakoRNAunannotated2.rds") # 30 PC
...
#subset_id <- 'cell_type'
cluster_order <- c('CB','CjS',
                   'CSB','CSSCs','LNPCs',
                   'LPCs','StC','Ves',
                   'IC','MEC','FCECs')
# Re-level object@ident
#seur_obj$cell_type <- factor(x = seur_obj$cell_type, levels = cluster_order)
#seur_obj_all$cell_type <- factor(x = seur_obj_all$cell_type, levels = cluster_order)
...
dev.off()
# 
saveRDS(seur_obj, file = "/ceph/rimlsfnwi/data/moldevbio/zhou/jarts/R/scRNA-seq/lakoRNAannotated.rds")
...
#pseudobulk_df2 <- do.call(cbind, apply(pseudobulk_df, 2, function(x) data.frame(x,x)))
...
#dds <- DESeqDataSetFromMatrix(pseudobulk_df, colData = rownames(pseudobulk_df))
#interesting genes
