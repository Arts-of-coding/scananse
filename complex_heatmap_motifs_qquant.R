...
lakoqnormmotifs <- read.table(file = '/ceph/rimlsfnwi/data/moldevbio/zhou/jarts/data/lako2021/motif_analysis/quantile_sig_filt/joined.txt', sep = ' ', header = TRUE, row.names=1)
lakoqnormmotifs <- lakoqnormmotifs[,1:9]
...
x$genes[x$genes == "TP53"] <- "TP63"
sum(x$genes == "TP53")

lakovstmotifs <- read.table(file = '/ceph/rimlsfnwi/data/moldevbio/zhou/jarts/data/lako2021/motif_analysis/quantile_sig_filt/joined.txt', sep = ' ', header = TRUE, row.names=1)
lakovstmotifs <- lakovstmotifs[,1:9]
selector2 <- unique(lakovstmotifs[c("genes")])
names2 <- rownames(selector2)
a <- subset(lakovstmotifs, rownames(lakovstmotifs) %in% names2)

# because the motif that contains TP63 is selected with TP73 and TP53 (that are not expressed), this value is manually changed
a$genes[a$genes == "TP53"] <- "TP63"
sum(a$genes == "TP63")
...
#ht3 = Heatmap(maty, col = f3, cluster_columns = F, name = "motifs z-score qnorm")

#matx <- matx[rownames(maty),colnames(maty)]
#matz <- matz[rownames(maty),colnames(maty)]


ht1 = Heatmap(matz, col= f2, cluster_rows = T, cluster_columns = T, name = "Z-score RNA count")
...
#ht3 = Heatmap(matx, col = f3, cluster_columns = F, cluster_rows=F, clustering_distance_rows = "euclidian",name = "motifs z-score qnorm")
...
for (i in colnames(mattest)) {
  ...
  htlist1 <- ht1 + rowAnnotation(link = anno_mark(at =  vec3,labels = vec))
  htlist2 <- ht3 + rowAnnotation(link = anno_mark(at =  vec3,labels = vec))
  draw(htlist1, column_title = i)
